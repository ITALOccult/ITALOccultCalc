using System;
using System.Collections.Generic;
using System.Drawing;
using System.Drawing.Imaging;
using System.IO;

namespace GifCreator
{
	public class GifClass
	{
		public GIFVersion m_Version;

		public Bitmap m_Image;

		public List<byte> m_GifSignature = new List<byte>();

		public List<byte> m_ScreenDescriptor = new List<byte>();

		public List<byte> m_ColorTable = new List<byte>();

		public List<byte> m_ImageDescriptor = new List<byte>();

		public List<byte> m_ImageData = new List<byte>();

		public void LoadGifPicture(string filename)
		{
			LoadGifPicture((Bitmap)Image.FromFile(filename));
		}

		public void LoadGifPicture(Bitmap gifPicture)
		{
			MemoryStream memoryStream = new MemoryStream();
			m_Image = gifPicture;
			m_Image.Save(memoryStream, ImageFormat.Gif);
			List<byte> gifData = new List<byte>(memoryStream.ToArray());
			if (!AnalyzeGifSignature(gifData))
			{
				throw new Exception("File is not a gif!");
			}
			AnalyzeScreenDescriptor(gifData);
			GIFBlockType typeOfNextBlock = GetTypeOfNextBlock(gifData);
			while (true)
			{
				switch (typeOfNextBlock)
				{
				case GIFBlockType.ImageDescriptor:
					AnalyzeImageDescriptor(gifData);
					break;
				case GIFBlockType.Extension:
					ThrowAwayExtensionBlock(gifData);
					break;
				case GIFBlockType.Trailer:
					return;
				}
				typeOfNextBlock = GetTypeOfNextBlock(gifData);
			}
		}

		private bool AnalyzeGifSignature(List<byte> gifData)
		{
			for (int i = 0; i < 6; i++)
			{
				m_GifSignature.Add(gifData[i]);
			}
			gifData.RemoveRange(0, 6);
			string text = new string(m_GifSignature.ConvertAll<char>(ByteToChar).ToArray());
			if (text == GIFVersion.GIF89a.ToString())
			{
				m_Version = GIFVersion.GIF89a;
			}
			else
			{
				if (!(text == GIFVersion.GIF87a.ToString()))
				{
					return false;
				}
				m_Version = GIFVersion.GIF87a;
			}
			return true;
		}

		private char ByteToChar(byte b)
		{
			return (char)b;
		}

		private void AnalyzeScreenDescriptor(List<byte> gifData)
		{
			for (int i = 0; i < 7; i++)
			{
				m_ScreenDescriptor.Add(gifData[i]);
			}
			gifData.RemoveRange(0, 7);
			if ((m_ScreenDescriptor[4] & 0x80u) != 0)
			{
				int num = m_ScreenDescriptor[4] & 7;
				int num2 = 3 * (int)Math.Pow(2.0, num + 1);
				for (int j = 0; j < num2; j++)
				{
					m_ColorTable.Add(gifData[j]);
				}
				gifData.RemoveRange(0, num2);
			}
			m_ScreenDescriptor[4] = (byte)(m_ScreenDescriptor[4] & 0x7Fu);
		}

		private GIFBlockType GetTypeOfNextBlock(List<byte> gifData)
		{
			return (GIFBlockType)gifData[0];
		}

		private void AnalyzeImageDescriptor(List<byte> gifData)
		{
			for (int i = 0; i < 10; i++)
			{
				m_ImageDescriptor.Add(gifData[i]);
			}
			gifData.RemoveRange(0, 10);
			if ((m_ImageDescriptor[9] & 0x80u) != 0)
			{
				int num = m_ImageDescriptor[9] & 7;
				int num2 = 3 * (int)Math.Pow(2.0, num + 1);
				m_ColorTable.Clear();
				for (int j = 0; j < num2; j++)
				{
					m_ColorTable.Add(gifData[j]);
				}
				gifData.RemoveRange(0, num2);
			}
			else
			{
				int num3 = m_ScreenDescriptor[4] & 7;
				m_ImageDescriptor[9] = (byte)(m_ImageDescriptor[9] & 0xF8u);
				m_ImageDescriptor[9] = (byte)(m_ImageDescriptor[9] | num3);
			}
			m_ImageDescriptor[9] = (byte)(m_ImageDescriptor[9] | 0x80u);
			GetImageData(gifData);
		}

		private void GetImageData(List<byte> gifData)
		{
			m_ImageData.Add(gifData[0]);
			gifData.RemoveAt(0);
			while (gifData[0] != 0)
			{
				int num = gifData[0];
				for (int i = 0; i <= num; i++)
				{
					m_ImageData.Add(gifData[i]);
				}
				gifData.RemoveRange(0, num + 1);
			}
			m_ImageData.Add(gifData[0]);
			gifData.RemoveAt(0);
		}

		private void ThrowAwayExtensionBlock(List<byte> gifData)
		{
			gifData.RemoveRange(0, 2);
			while (gifData[0] != 0)
			{
				gifData.RemoveRange(0, gifData[0] + 1);
			}
			gifData.RemoveAt(0);
		}
	}
}
