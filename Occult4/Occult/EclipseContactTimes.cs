...
namespace Occult
{
	internal class EclipseContactTimes : IComparable
	{
		...
		public int CompareTo(object other)
		{
			...
			{
				0 => siteName.CompareTo(((EclipseContactTimes)other).SiteName), 
				1 => longitude.CompareTo(((EclipseContactTimes)other).Longitude), 
				2 => ((EclipseContactTimes)other).Latitude.CompareTo(latitude), 
				_ => 0, 
			};
		}
		...
		public string ToString(bool Abbreviated)
		{
			...
			if (latitude < 100.0)
			{
				...
				stringBuilder.AppendFormat("{0,6:F0} ", altitude);
