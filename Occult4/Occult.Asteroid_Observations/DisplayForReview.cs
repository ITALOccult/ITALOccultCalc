...
{
	public class DisplayForReview : Form
	{
		private IContainer components;

		private Label label5;

		internal ListBox lstForReview;
		...
		public DisplayForReview()
		{
			InitializeComponent();
		}

		private void DisplayForReview_Load(object sender, EventArgs e)
		{
		}

		private void lstForReview_MouseDown(object sender, MouseEventArgs e)
		{
			...
			try
			{
				string text = lstForReview.get_SelectedItem().ToString();
				...
				string asteroidNumber = text.Substring(13, 6);
				string asteroidName = text.PadRight(40).Substring(20, 16).Trim();
		...
		private void DisplayForReview_Resize(object sender, EventArgs e)
		{
			if (((Control)this).get_Width() < 358)
			{
				((Control)this).set_Width(358);
			}
			((Control)lstForReview).set_Width(((Control)this).get_Width() - 21);
			if (((Control)this).get_Height() < 100)
			{
				((Control)this).set_Height(120);
			}
			((Control)lstForReview).set_Height(((Control)this).get_Height() - 74);
		}
		...
		private string CollectEvents(bool NoPlusMinus, bool SelectedLinesOnly)
		{
			...
			int count = lstForReview.get_Items().get_Count();
			if (SelectedLinesOnly)
			{
				for (int i = 0; i < lstForReview.get_SelectedItems().get_Count(); i++)
				{
					string value = lstForReview.get_SelectedItems().get_Item(i).ToString()!.Replace("±", " ");
			...
			else
			{
				...
				{
					string value2 = lstForReview.get_Items().get_Item(j).ToString()!.Replace("±", " ");
		...
		private void InitializeComponent()
		{
			...
			((Control)lstForReview).set_Font(new Font("Courier New", 8.25f, FontStyle.Regular, GraphicsUnit.Point, 0));
			((ListControl)lstForReview).set_FormattingEnabled(true);
			lstForReview.set_ItemHeight(14);
			((Control)lstForReview).set_Location(new Point(1, 31));
			((Control)lstForReview).set_Name("lstForReview");
			lstForReview.set_SelectionMode((SelectionMode)3);
			((Control)lstForReview).set_Size(new Size(614, 340));
			((Control)lstForReview).set_TabIndex(22);
			((Control)lstForReview).add_MouseDown(new MouseEventHandler(lstForReview_MouseDown));
			...
			((Control)menuStrip1).set_Size(new Size(619, 24));
			...
			((Form)this).set_ClientSize(new Size(619, 375));
			((Control)this).get_Controls().Add((Control)(object)label5);
			((Control)this).get_Controls().Add((Control)(object)lstForReview);
			((Control)this).get_Controls().Add((Control)(object)menuStrip1);
			((Control)this).set_Name("DisplayForReview");
			((Control)this).set_Text("Display events marked for  ForReview");
			((Form)this).add_Load((EventHandler)DisplayForReview_Load);
			((Control)this).add_Resize((EventHandler)DisplayForReview_Resize);
