...
{
	public class DisplayNoMainBody : Form
	{
		private IContainer components;

		private Label label5;

		internal ListBox lstNoMainBody;
		...
		public DisplayNoMainBody()
		{
			InitializeComponent();
		}

		private void lstNoMainBody_MouseDown(object sender, MouseEventArgs e)
		{
			...
			try
			{
				string text = lstNoMainBody.get_SelectedItem().ToString();
				if (text.Trim().Length < 10)
				{
					ErrorMessage(((Control)this).get_Left(), ((Control)this).get_Top());
					return;
				}
				if (text.Substring(0, 3) == "Eve")
				{
					ErrorMessage(((Control)this).get_Left(), ((Control)this).get_Top());
					return;
				}
				string asteroidNumber = text.Substring(14, 7);
				string asteroidName = text.PadRight(41).Substring(22, 16).Trim();
				int.TryParse(text.Substring(0, 4), out var result);
				int month = 1;
				string text2 = text.Substring(5, 3);
				for (int i = 1; i < 13; i++)
				{
				...
				int.TryParse(text.Substring(9, 2), out var result2);
		...
		private void DisplayNoMainBody_Resize(object sender, EventArgs e)
		{
			...
			((Control)lstNoMainBody).set_Height(((Control)this).get_Height() - 74);
		}
		...
		private string CollectEvents(bool NoPlusMinus, bool SelectedLinesOnly)
		{
			...
			int count = lstNoMainBody.get_Items().get_Count();
			if (SelectedLinesOnly)
			{
				for (int i = 0; i < lstNoMainBody.get_SelectedItems().get_Count(); i++)
				{
					string value = lstNoMainBody.get_SelectedItems().get_Item(i).ToString()!.Replace("±", " ");
			...
			else
			{
				...
				{
					string value2 = lstNoMainBody.get_Items().get_Item(j).ToString()!.Replace("±", " ");
		...
		private void InitializeComponent()
		{
			...
			((Control)label5).set_Location(new Point(115, 4));
			...
			((Control)lstNoMainBody).set_Font(new Font("Courier New", 8.25f, FontStyle.Regular, GraphicsUnit.Point, 0));
			((ListControl)lstNoMainBody).set_FormattingEnabled(true);
			lstNoMainBody.set_ItemHeight(14);
			((Control)lstNoMainBody).set_Location(new Point(1, 28));
			((Control)lstNoMainBody).set_Name("lstNoMainBody");
			lstNoMainBody.set_SelectionMode((SelectionMode)3);
			((Control)lstNoMainBody).set_Size(new Size(337, 340));
			((Control)lstNoMainBody).set_TabIndex(22);
			((Control)lstNoMainBody).add_MouseDown(new MouseEventHandler(lstNoMainBody_MouseDown));
			...
			((Control)menuStrip1).set_Size(new Size(340, 24));
			...
			((Form)this).set_ClientSize(new Size(340, 369));
			((Control)this).get_Controls().Add((Control)(object)label5);
			((Control)this).get_Controls().Add((Control)(object)lstNoMainBody);
			((Control)this).get_Controls().Add((Control)(object)menuStrip1);
			((Control)this).set_Name("DisplayNoMainBody");
			((Control)this).set_Text("Events where Main body not detected");
			((Control)this).add_Resize((EventHandler)DisplayNoMainBody_Resize);
