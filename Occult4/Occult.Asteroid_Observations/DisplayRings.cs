...
{
	public class DisplayRings : Form
	{
		...
		private string CollectEvents(bool NoPlusMinus, bool SelectedLinesOnly)
		{
			...
			int count = lstRings.get_Items().get_Count();
			if (SelectedLinesOnly)
			{
				for (int i = 0; i < lstRings.get_SelectedItems().get_Count(); i++)
				{
					string value = lstRings.get_SelectedItems().get_Item(i).ToString()!.Replace("±", " ");
			...
			else
			{
				...
				{
					string value2 = lstRings.get_Items().get_Item(j).ToString()!.Replace("±", " ");
		...
		private void lstRings_MouseDown(object sender, MouseEventArgs e)
		{
			...
			try
			{
				string text = lstRings.get_SelectedItem().ToString();
		...
		private void DisplayRings_Resize(object sender, EventArgs e)
		{
			...
			((Control)lstRings).set_Height(((Control)this).get_Height() - 71);
		}
		...
		private void InitializeComponent()
		{
			...
			((Control)lstRings).set_Font(new Font("Courier New", 8.25f, FontStyle.Regular, GraphicsUnit.Point, 0));
			((ListControl)lstRings).set_FormattingEnabled(true);
			lstRings.set_ItemHeight(14);
			((Control)lstRings).set_Location(new Point(1, 24));
			((Control)lstRings).set_Name("lstRings");
			lstRings.set_SelectionMode((SelectionMode)3);
			((Control)lstRings).set_Size(new Size(337, 326));
			((Control)lstRings).set_TabIndex(0);
			((Control)lstRings).add_MouseDown(new MouseEventHandler(lstRings_MouseDown));
			...
			((Control)label5).set_Location(new Point(111, 6));
			...
			((Control)menuStrip1).set_Size(new Size(342, 24));
			...
			((ToolStripItem)withRingsToolStripMenuItem).set_Size(new Size(83, 20));
			((ToolStripItem)withRingsToolStripMenuItem).set_Text("with Rings...");
			...
			((Form)this).set_ClientSize(new Size(342, 358));
			((Control)this).get_Controls().Add((Control)(object)label5);
			((Control)this).get_Controls().Add((Control)(object)lstRings);
			((Control)this).get_Controls().Add((Control)(object)menuStrip1);
			((Form)this).set_MainMenuStrip(menuStrip1);
			((Control)this).set_Name("DisplayRings");
			((Control)this).set_Text("Observations of ring events");
			((Control)this).add_Resize((EventHandler)DisplayRings_Resize);
