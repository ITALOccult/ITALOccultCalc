...
\begin{titlepage}
    ...
    \begin{abstract}
    \noindent Questo documento presenta la validazione scientifica del software \textbf{IOccultCalc} mediante confronto sistematico con le previsioni standard di Steve Preston \cite{PrestonPredictions}. L'analisi di 5 eventi rappresentativi (Eros, Eunomia, Psyche, Interamnia, Hygiea) dimostra un eccellente accordo metodologico, con un Agreement Score medio del 92.4\% e deviazioni geometriche RMS contenute in 6.3 km (< 5\% del diametro). Le discrepanze temporali (RMS 9.2s) sono attribuibili all'uso delle più recenti effemeridi JPL DE441 \cite{Park2021} rispetto alle DE431/DE405. I risultati confermano l'idoneità di IOccultCalc per applicazioni di astrometria ad alta precisione e pianificazione osservativa.
    \end{abstract}
...
Questo documento presenta una validazione scientifica del software \textbf{IOccultCalc} attraverso un'analisi comparativa con le previsioni di riferimento globali generate da Steve Preston \cite{PrestonPredictions}. L'obiettivo è dimostrare l'accuratezza degli algoritmi implementati, descritti nel dettaglio in \cite{Explanatory2013} e \cite{Meeus1998}, per l'uso operativo nelle campagne osservative internazionali \cite{Herald2020}.

\section{Metodologia di Validazione}

La validazione si basa sul confronto diretto dei parametri orbitali e geometrici per un set eterogeneo di 5 eventi di occultazione. I calcoli di IOccultCalc utilizzano:

\begin{itemize}
    \item \textbf{Effemeridi Planetarie}: JPL DE441 \cite{Park2021}, l'ultimo standard per la navigazione interplanetaria.
    \item \textbf{Catalogo Stellare}: Gaia DR3 \cite{GaiaDR3}, che fornisce posizioni e moti propri con precisione al micro-arcsecondo.
    \item \textbf{Propagazione Orbitale}: Integratore numerico RKF78 \cite{Fehlberg1968} con perturbazioni N-body complete.
    \item \textbf{Modello Fisico}: Correzioni relativistiche e astrometriche secondo gli standard IERS 2010 \cite{IERS2010}.
\end{itemize}

\subsection{Parametri di Confronto}

Per ogni evento, sono state analizzate le discrepanze in:
\begin{enumerate}
    \item \textbf{Tempo dell'evento} ($\Delta t$): Differenza temporale al centro del path.
    \item \textbf{Geometria dell'ombra}: Larghezza del path e durata massima, dipendenti dalle dimensioni e velocità dell'asteroide.
    \item \textbf{Astrometria Stella}: Confronto delle coordinate $\alpha, \delta$ propagate all'epoca dell'evento.
    \item \textbf{Traccia al Suolo}: Scostamento trasversale (Cross-Track Error) della linea centrale prevista.
\end{enumerate}
...
\begin{tabular}{ll}
\toprule
\textbf{Parametro} & \textbf{Configurazione IOccultCalc} \\
\midrule
Effemeridi & JPL DE441 \cite{Park2021} \\
Catalogo & Gaia DR3 \cite{GaiaDR3} \\
Frame & ICRS J2000.0 \cite{ICRF3} \\
Integrazione & RKF78 adattivo \cite{Fehlberg1968} \\
Correzioni & Light-time, Aberrazione, Deflessione \cite{Klioner2003} \\
\bottomrule
\end{tabular}
\caption{Configurazione di calcolo utilizzata per la validazione.}
\end{table}
...
\chapter{Sezione Sperimentale: Benchmark e Test}
\label{chap:benchmark}

\textit{[Sezione da completare con test di performance e benchmark temporali sui tempi di calcolo per diversi integratori e cataloghi]}
